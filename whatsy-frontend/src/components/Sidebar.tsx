...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside data-testid="sidebar" className="w-full md:w-[380px] lg:w-[420px] h-full flex flex-col bg-[#111b21] border-r border-[#222e35] select-none">
      {/* Sidebar Header */}
      <header className="h-[60px] bg-[#202c33] px-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <div className="relative">
            <img
              src={userAvatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100'}
              alt="My Profile"
              className="w-10 h-10 rounded-full object-cover ring-1 ring-emerald-500/50 cursor-pointer"
            />
            <span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 rounded-full border-2 border-[#202c33]" />
          </div>
          <span className="font-semibold text-sm text-[#e9edef] tracking-tight">Chats</span>
        </div>
        ...
        <div className="relative flex items-center bg-[#202c33] rounded-lg px-3 py-1.5 focus-within:ring-1 focus-within:ring-[#00a884]">
          <svg
            className={`w-4 h-4 mr-3 transition-colors ${
              isSearchFocused ? 'text-[#00a884]' : 'text-[#8696a0]'
            }`}
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
          ...
          {searchQuery && (
            <button
              ...
              className="text-[#8696a0] hover:text-[#e9edef] ml-1"
            >
      ...
      {/* Filter Pills */}
      <div className="px-3 py-1.5 flex items-center gap-1.5 overflow-x-auto border-b border-[#222e35] scrollbar-none">
        {filters.map((f) => (
          <button
            key={f.id}
            type="button"
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-[#202c33]/40">
        {conversations.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-[#8696a0] text-sm p-4 text-center">
          ...
          conversations.map((conv) => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 px-3 py-3 cursor-pointer transition relative hover:bg-[#202c33]/70 ${
              ...
              >
                ...
                <div className="relative shrink-0">
                  ...
                  {conv.participant.isOnline && (
                    <span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 rounded-full ring-2 ring-[#111b21]" />
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <h3 className="font-medium text-sm text-[#e9edef] truncate">
                      {conv.participant.displayName}
                    </h3>
                    <span
                      className={`text-[11px] shrink-0 ml-2 ${
                        conv.unreadCount > 0 ? 'text-[#00a884] font-semibold' : 'text-[#8696a0]'
                      }`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {conv.isPinned && (
                        <svg className="w-3.5 h-3.5 text-[#8696a0]" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M16 9V4h1c.55 0 1-.45 1-1s-.45-1-1-1H7c-.55 0-1 .45-1 1s.45 1 1 1h1v5c0 1.66-1.34 3-3 3v2h5.97v7l1 1 1-1v-7H19v-2c-1.66 0-3-1.34-3-3z"/>
                        </svg>
                      )}
                      {conv.unreadCount > 0 && (
                        <span className="bg-[#00a884] text-[#111b21] font-bold text-[11px] min-w-[20px] h-5 px-1.5 rounded-full flex items-center justify-center">
                          {conv.unreadCount}
                        </span>
                      )}
                    </div>
